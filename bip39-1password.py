# -----------------------------------------------------------------------------
# bip39-1password will accept a list of seed words from the BIP39 word list, 
# and decode back as a secret key for a 1password account.  You should use this
# script to verify that you can recover your secret key before stamping the
# output from 1password-bip39 to metal.
#
# For example, if you use the following set of words
#    embark crumble loan eternal boring abuse patrol mention manual snack
#    icon bean own skull govern harsh master december antique still palm
#    ride arm theme
# The output should return the secret key as
...
# helper to determine if provided word is in the list, and if so, returns
# the index
def getWordNum(theword):
    l = len(wl)
    for i in range(l):
        if wl[i] == theword:
            return i
...
for m in range(24):
    n = getWordNum(r[m])
    if n > -1:
        print(f"Valid word {m+1}: {r[m]} is word #{n}")
    while n == -1:
        if len(r[m]) > 0:
            print(f"Word '{r[m]}' in position {m+1} is invalid!!!")
        print(f"Enter word {m+1}:")
        r[m] = input().strip().lower()
        n = getWordNum(r[m])
...
for h in range(34):
    ...
    e = chr(n+48)
    y = y + e
    if h in [1,7,13,18,23,28]:
        y = y + "-"

# Dump the formatted string representing the secret key
print(f"The secret key is: {y}")
